$(function() {
	...
	if (typeof storageFiles.ownerPhoto === "undefined" || typeof info === "undefined") {
		...
		ownerPhoto
				.load(function() {
					var ownerPhoto_element = document
							.getElementById("owner_photo_img");
					...
					// 在canvas中绘制图片
					imgContext
							.drawImage(ownerPhoto_element, 0, 0,
									ownerPhoto_element.width,
									ownerPhoto_element.height);
					// 将图片保存为Data URI
					storageFiles.ownerPhoto = imgCanvas.toDataURL("image/png");
					// 将JSON保存到本地存储中
					try {
						sessionStorage.setItem("storageFiles", JSON
								.stringify(storageFiles));
					} catch (e) {
